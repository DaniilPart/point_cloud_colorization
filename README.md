...
### Gaussian Splatting roadmap

A future improvement is to add a standard offline Gaussian Splatting reconstruction path after the existing ROS colorization pipeline. The current ROS nodes should still be used to collect synchronized camera, LiDAR, odometry, and TF data. Gaussian Splatting should then be trained after a run, using the recorded data as a scene dataset.

Planned offline workflow:

1. Record a mapping run:
   - camera images
   - camera calibration from `CameraInfo`
   - odometry / TF poses
   - optional colorized point cloud map from the current pipeline

2. Export a Gaussian Splatting dataset:
   - save selected camera frames to an `images/` directory
   - save camera intrinsics
   - save one camera pose per exported image
   - convert ROS poses into the camera/world convention required by the training tool
   - optionally export the current point cloud map as an initial point cloud

3. Train a standard Gaussian Splatting model:
   - use an existing trainer such as GraphDECO Gaussian Splatting, Nerfstudio Splatfacto, or OpenSplat
   - use the exported images and camera poses as training input
   - use the point cloud map as an optional geometry prior if the selected tool supports it

4. Export the reconstruction result:
   - save the trained Gaussian model
   - generate a viewer-compatible output
   - package the result together with the map and trajectory files

5. Integrate with post-processing:
   - add a script such as `export_splat_dataset`
   - add a script such as `train_splats`
   - optionally extend `postprocess_map` to run the full export/train/package flow

Expected result:

- the robot can record data using the existing ROS pipeline
- Gaussian Splatting can produce a higher-quality visual reconstruction after the run
- the first implementation stays offline and avoids changing the real-time mapper

Later, a separate real-time or near-real-time splatting mode can be investigated by rendering the live voxel / point cloud map as lightweight Gaussian-like splats.
